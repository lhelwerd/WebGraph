...
public class ParallelBreadthFirstBetweennessCentralityTest {
	...
	public void testCycle() throws InterruptedException {
		for( int size: new int[] { 10, 50, 100 } ) {	
			...
			DoubleArrays.fill( expected, ( size - 1 ) * ( size - 2 ) / 2.0 );
	...
	public void testRandom() throws InterruptedException {
		for ( double p: new double[] { .1, .2, .5, .7 } )
			for( int size: new int[] { 10, 50, 100 } ) {
				...
				edu.uci.ics.jung.algorithms.importance.BetweennessCentrality<Integer, Long> jungParallelBreadthFirstBetweennessCentrality = new edu.uci.ics.jung.algorithms.importance.BetweennessCentrality<Integer, Long>( jungGraph );
